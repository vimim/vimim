...
" ----------------------------------------------
    ...
    if empty(keyboard) && empty(chinese)
        return []
    endif
    " ------------------------------------------
    let lines = s:vimim_load_datafile(0)
...
if a:start
    ...
    " ----------------------------------------------------
    if s:chinese_frequency > 0 && start_row >= s:start_row_before
...
" -------------------------------------
function! s:vimim_define_mapping_keys()
" -------------------------------------
...
silent!call s:vimim_define_mapping_keys()
