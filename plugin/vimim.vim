...
" -------------------------------
function! s:vimim_unihan(english)
" -------------------------------
    let key = a:english
...
" -------------------------
    let vim1 = s:vimim_unihan('vim1')
    let vim2 = s:vimim_unihan('vim2') . vim1
    let vim3 = s:vimim_unihan('vim3')
    let vim4 = s:vimim_unihan('vim4')
    let vim5 = s:vimim_unihan('vim5')
    " ------------------------------------
    let eggs  = ["vi    " . vim1 ]
    let eggs += ["vim   " . vim2 ]
    let eggs += ["vim   " . vim3 ]
...
" ---------------------------
    ...
    if empty(option)
        ...
        if !empty(s:unicode_digit_lines)
            let filter = s:path . "vimim.filter.txt"
            if option !~ filter
...
" ----------------------------------
function! <SID>ChineseModeAlwaysOn()
" ----------------------------------
    let action = s:vimim_chinesemode(0)
...
" --------------------------
    let action = s:vimim_chinesemode(1)
    sil!exe 'sil!return "' . action . '"'
endfunction
...
" ----------------------------
    ...
    " ------------------------------------
    if s:pinyin_plus_filter > 0 && s:ui.im == 'pinyin'
        let im_digit = s:vimim_unihan('digit')
        let pinyin = s:vimim_unihan('pinyin')
        let s:ui.statusline = pinyin . s:plus . im_digit
        return s:vimim_get_im_unihan()
    endif
    " ------------------------------------
...
" -------------------------------------------------
    ...
    " ---------------------------
        ...
        " -------------------------------------------------
        let extra_text = menu
        if s:pinyin_plus_filter > 0 && len(s:menu_digit_as_filter) > 0
...
" -----------------------------------
    ...
    \|| s:chinese_input_mode=~ 'onekey')
        ...
        " -----------------------------------------
        if s:pinyin_plus_filter > 0 || s:vimim_custom_skin == 2
            let labeling = label2
        else
            let labeling .= label2
        endif
...
" http://vimim-data.googlecode.com/svn/trunk/data/vimim.filter.txt
" -----------------------------------------------------------------
    let datafile = s:path . "vimim.filter.txt"
    if filereadable(datafile) && empty(s:unicode_digit_lines)
        let s:pinyin_plus_filter = 1
...
" ---------------------------------------------------
    if empty(s:pinyin_plus_filter) | return | endif
...
" ---------------------------------------------------
    ...
    if s:pinyin_plus_filter > 0 && len(keyboards) > 1
...
" -------------------------------------------
    ...
    for chinese in words
        if chinese =~ '\w'
            ...
            let digit = get(s:unicode_digit_cache[key],0)
            let head .= digit[:0]
            let tail = digit[1:]
        endif
    endfor
...
" ---------------------------------------
    ...
    if s:vimim_shuangpin_abc > 0
        let chinese = s:vimim_unihan('abc')
    elseif s:vimim_shuangpin_microsoft > 0
        let chinese = s:vimim_unihan('microsoft')
        let keycode = "[0-9a-z'.;]"
    elseif s:vimim_shuangpin_nature > 0
        let chinese = s:vimim_unihan('nature')
    elseif s:vimim_shuangpin_plusplus > 0
        let chinese = s:vimim_unihan('plusplus')
    elseif s:vimim_shuangpin_purple > 0
        let chinese = s:vimim_unihan('purple')
        let keycode = "[0-9a-z'.;]"
    elseif s:vimim_shuangpin_flypy > 0
        let chinese = s:vimim_unihan('flypy')
    else
        let s:shuangpin_flag = 0
    endif
    " ------------------------------------
...
" ---------------------------------------------------
    ...
    if a:more > 0 && s:pinyin_plus_filter > 0
...
" ------------------------------------------------
    ...
    let msg = "vim datafile.pinyin.vimim datafile.digit.vimim"
...
" --------------------------------------
    ...
    \&& empty(s:backend.datafile[im].lines)
...
" -------------------------------------------------
    ...
    for im in valid_directoires
        ...
        " ------------------------------------------------------
        if im =~ 'digit' | let s:pinyin_plus_filter=2 | endif
    endfor
endfunction
